...
public class DfpModule extends KrollModule {
	...
	// PublisherAdRequest.DEVICE_ID_EMULATOR
	// as the constant, but I got nasty build errors like
	// [exec] jni/ti.dfp.DfpModule.cpp: In static member function 'static
	// v8::Handle<v8::FunctionTemplate> ti::dfp::DfpModule::getProxyTemplate()':
	// [exec] jni/ti.dfp.DfpModule.cpp:113:3: error: no matching function for
	// call to 'v8::String::New(_jobject*&)'
			...
			public void run() {
				try {
					AdvertisingIdClient.Info adInfo = AdvertisingIdClient
							.getAdvertisingIdInfo(TiApplication.getInstance());
					finished(adInfo);
					return;
				} catch (IOException e) {
					Log.w(LCAT,
							"[ti.dfp] call to getAdvertisingInfo() failed; could not make connection to google play services");
				} catch (IllegalStateException e) {
					Log.w(LCAT,
							"[ti.dfp] call to getAdvertisingInfo() failed; method called on main thread");
				} catch (GooglePlayServicesNotAvailableException e) {
					Log.w(LCAT,
							"[ti.dfp] call to getAdvertisingInfo() failed; google play services not available");
				} catch (GooglePlayServicesRepairableException e) {
					Log.w(LCAT,
							"[ti.dfp] call to getAdvertisingInfo() failed; recoverable error connecting to google play services");
				}
	...
	private void finished(final AdvertisingIdClient.Info adInfo) {
		...
		if (hasListeners("ready")) {
			...
			kd.put("id", adInfo.getId());
			kd.put("isLimitAdTrackingEnabled",
